...
fn main() -> io::Result<()> {	
	...
	for _i in 0..female_manatees.len() {
		if !impossible {
			...
			//dbg!("finding a pair for female of size {} and age {}", first_female.size, first_female.age);
			...
			if no_pair {
				...
				for j in 0..female_manatees.len() {
					if female_manatees[j].size > first_male.size { // If there is a female manatee that is larger than the male
						//dbg!("removing female of size {} and age {}", female_manatees[j].size, female_manatees[j].age);
						let female_index = female_manatees.remove(j).index; // Put female into answer
						female_answer.push(female_index);
						//dbg!("removing male of size {} and age {}", male_manatees[j].size, male_manatees[j].age);
